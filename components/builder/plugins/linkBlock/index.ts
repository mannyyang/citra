...
const plugin: Plugin = (editor: Editor) => {
  ...
  Components.addType(ComponentTypes.NtvbLinkBlock, {
    isComponent: (el: HTMLElement) => {
      if (el && el.classList) return el.classList.contains(ComponentClasses.NtvbLinkBlock);
    },
    model: {
      defaults: {
        classes: [ComponentClasses.NtvbLinkBlock],
        style: {
  ...
  BlockManager.add(BlockIdentifies.Link, {
    ...
    content: {
      type: ComponentTypes.NtvbLinkBlock
    }
  });
};
