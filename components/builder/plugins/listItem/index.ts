...
const plugin: Plugin = (editor: Editor) => {
  ...
  Components.addType(ComponentTypes.NtvbListItem, {
    isComponent: (el: HTMLElement) => {
      if (el && el.classList) {
        return el.classList.contains(ComponentClasses.NtvbListItem);
      }
    },
    model: {
      defaults: {
        classes: [ComponentClasses.NtvbListItem],
        tagName: 'li',
        style: LIST_ITEM_STYLE,
        draggable: `[data-gjs-type="${ComponentTypes.NtvbList}"]`
      }
    }
  });
